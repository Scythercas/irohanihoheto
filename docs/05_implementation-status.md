...
**目標は「体験版（共通パート＋桃果ルート）を公開する」こと**（D2②）。
そこから逆算した順序。**上から順にやれば、常に「遊べる状態」を保ったまま進む。**

### 第1段階 ― 骨格を完成させる（体験版の必須要件）

| # | 作業 | なぜ先か |
|---|---|---|
| 1 | **葵・翠・橙香・紫音との出会いシーンを書く**。`met_*` フラグを立てて5人全員をスケジュールに出す | **これで初めてゲームの全体像が見える。** 「1人に入れ込むと茜に届かない」というトレードオフが体感できるようになる。茜のデレ段階（`ifMetCount: 2`）も解放条件を満たせるようになる |
| 2 | **エンディング判定**を実装（`branch` ノード＋`THRESHOLD`）。個別／茜／サッドの3系統を分岐させる | 終わりが無いと「1周」できない。`finale_judge` は現在まだ仮 |
| 3 | **桃果ルートのデート2〜3回目**を書く | 体験版の主役ルートを最後まで通す |

**この3つが終われば、体験版が最初から最後まで遊べる。**
...
| # | 作業 |
|---|---|
| 9 | **残り5人の立ち絵**（各7表情）。手順は `docs/04_asset-guide.md` |
| 10 | **BGM・効果音の選定**（DOVA-SYNDROME／効果音ラボ）。使ったら即 `06_credits.md` に記録 |
| 11 | **ボイス**（VOICEVOX＋葵は知人収録。まずテスト録音5本） |
| 12 | **足りない背景の洗い出し**。`npm run check:scenario` が未登録キーを教えてくれる |
...
| 14 | **itch.io に体験版ページを作成**。スクリーンショット・紹介文・タグ |
| 15 | **クレジット画面をゲーム内に実装**（`06_credits.md` §6 の文面） |
| 16 | 完成版のタイミングで**ノベルゲームコレクション**等への登録を判断（T7） |

### 第5段階 ― 本編（体験版の反応を見てから）

残り4ルート（葵・翠・橙香・紫音）と茜ルートを書き切る。
